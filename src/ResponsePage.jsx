...
export default function ResponsePage() {
  ...
  function updateSize(index, value) {
    setItems((previous) =>
      previous.map((item, i) =>
        i === index ? { ...item, confirmed_size: value } : item
      )
    );
  }

  async function submitForm() {
    const missing = items.some((item) => !String(item.confirmed_size || "").trim());
    ...
    try {
      ...
      setMessage("Merci, tes tailles ont bien été confirmées.");
    } catch (error) {
      setMessage(error.message);
    }
  }
  ...
  if (loading) {
    return <PageShell>Chargement du formulaire...</PageShell>;
  }

  if (!id || !emailLog) {
    return <PageShell>{message || "Formulaire introuvable."}</PageShell>;
  }
    ...
    <div className="min-h-screen bg-neutral-950 text-neutral-100 p-6">
      <div className="max-w-3xl mx-auto bg-neutral-900 border border-neutral-800 rounded-3xl p-6 shadow-xl">
        <div className="mb-6">
          ...
          <h1 className="text-3xl font-bold">Confirmation des tailles</h1>
        ...
        <div className="rounded-2xl bg-neutral-950 border border-neutral-800 p-4 mb-6 text-sm">
          <div><strong>Compétiteur :</strong> {emailLog.competitor || "—"}</div>
          <div><strong>Dojo :</strong> {emailLog.dojo || "—"}</div>
          <div><strong>Équipe :</strong> {emailLog.equipe || "—"}</div>
          <div><strong>Courriel :</strong> {emailLog.recipient_email || "—"}</div>
        </div>

        <div className="space-y-4">
          {items.map((item, index) => (
            <div key={item.id || index} className="rounded-2xl bg-neutral-950 border border-neutral-800 p-4">
              ...
              <label className="block space-y-2">
                <span className="text-sm font-semibold">Taille confirmée</span>
                <input
                  value={item.confirmed_size}
                  onChange={(event) => updateSize(index, event.target.value)}
                  placeholder="Ex. YL, M, 150, 10 ans..."
                  className="w-full bg-white text-black border border-neutral-300 rounded-xl p-3"
                />
              </label>
            </div>
          ))}
        ...
        <label className="block space-y-2 mt-6">
          <span className="text-sm font-semibold">Commentaire optionnel</span>
          <textarea
            value={comments}
        ...
        <button
          onClick={submitForm}
          className="mt-6 w-full bg-yellow-500 hover:bg-yellow-400 text-neutral-950 font-bold rounded-2xl p-4"
        >
